#include "anomaly_detection/slip_detection_imu.hpp"

using namespace anomaly_detection;


    SlipDetectionIMU::SlipDetectionIMU()
    ...
    {   
        ...
        this->declare_parameter<double>("threshold_angle", 100); //暂不考虑旋转打滑
        ...
        sync_->registerCallback(std::bind(&SlipDetectionIMU::callback, this, std::placeholders::_1, std::placeholders::_2));
        
        
        timer_ = this->create_wall_timer(std::chrono::milliseconds(options_.period),
                std::bind(&SlipDetectionIMU::timer_callback, this), timer_callback_group_);
        
    }

    
    void SlipDetectionIMU::callback(const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg,  
    ...
    void SlipDetectionIMU::timer_callback()
    {   
        {
            std::unique_lock<std::mutex> lock_msgs(msgs_que_mtx_);
        ...
        {   
            ...
            imu_ = std::make_shared<anomaly_detection::IMU>(Eigen::Vector3d(0, 0, -9.8), Eigen::Vector3d(0.015, 0, 0), Eigen::Vector3d::Zero());
            ...
            {   
                ...
                last_imu_time_ = current_imu_time_;
                exec_queue_.pop();
            }      
        }
